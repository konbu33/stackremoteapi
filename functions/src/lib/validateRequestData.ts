...
export class ValidateRequestData {
  ...
  static validateUid(requestDataObject: RequestData): Result<number> {
    const uid = requestDataObject.localUid;
  ...
  static validateRole(requestDataObject: RequestData): Result<number> {
    const roleName = requestDataObject.role;
    let roleId = 99;
  ...
  static validatePrivilegeExpireTime(
    requestDataObject: RequestData
  ): Result<number> {
    // 権限期限切れ時間を計算
    const expireTime = requestDataObject.privilegeExpireTime;
    let expireTimeInt = 3600;
