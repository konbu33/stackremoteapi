...
describe("responseData", () => {
  test("create ResponseDataState, and state getter", () => {
    ...
    // Then
    expect(responseDataState.state.channelName).toEqual(
      _responseData.channelName
    );
    ...
    expect(responseDataState.state.rtcIdTokenType).toEqual(
      _responseData.rtcIdTokenType
    );
    expect(responseDataState.state.role).toEqual(_responseData.role);
    expect(responseDataState.state.privilegeExpireTime).toEqual(
      _responseData.privilegeExpireTime
    );

    expect(responseDataState.state.firebaseAuthIdToken).toEqual(
      _responseData.firebaseAuthIdToken
    );
    expect(responseDataState.state.verifyFirebaseAuthUid).toEqual(
      _responseData.verifyFirebaseAuthUid
    );
    expect(responseDataState.state.rtcIdToken).toEqual(
      _responseData.rtcIdToken
    );
  });
  ...
  test("setRtcIdToken", () => {
    ...
    // When
    const newRtcIdToken = "newRtcIdToken002";
