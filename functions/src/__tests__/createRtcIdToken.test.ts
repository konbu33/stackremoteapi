...
describe("createRtcIdToken", () => {
  ...
  test("200 OK", async () => {
    ...
    const expectRole =
      data.role === "publisher" ? 1 : data.role === "audience" ? 2 : undefined;
    expect(JSON.parse(result.data).role).toEqual(expectRole);
    const aboutPrivilegeExpireTime =
      ValidateRequestData.calcPrivilegeExpireTime(data.privilegeExpireTime);
    expect(JSON.parse(result.data).privilegeExpireTime).toBeWithinRange(
      aboutPrivilegeExpireTime - 5000,
      aboutPrivilegeExpireTime + 500
    );

    expect(JSON.parse(result.data).firebaseAuthIdToken).toEqual(
      data.firebaseAuthIdToken
    );
    expect(JSON.parse(result.data).verifyFirebaseAuthUid).toEqual(
      JSON.parse(verifyFirebaseAuthIdTokenResult.data).uid
    );
    expect(JSON.parse(result.data).rtcIdToken).not.toEqual(
