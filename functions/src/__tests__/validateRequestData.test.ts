...
describe("validatePrivilegeExpireTime", () => {
  test("200 OK default expire", () => {
    ...
    // When
    const result =
      ValidateRequestData.validatePrivilegeExpireTime(requestDataObject);
    const expectPrivilegeExpireTime =
      ValidateRequestData.calcPrivilegeExpireTime(3600);
    console.log("result : ", result);
    ...
    expect(result.code).toEqual(200);
    expect(result.message).toContain("no problem");
    expect(result.data).toBeWithinRange(
      expectPrivilegeExpireTime - 5000,
      expectPrivilegeExpireTime + 5000
    );
  });
  ...
  test("200 OK", () => {
    ...
    const result =
      ValidateRequestData.validatePrivilegeExpireTime(requestDataObject);
    const expectPrivilegeExpireTime =
      ValidateRequestData.calcPrivilegeExpireTime(
        requestDataObject.privilegeExpireTime
      );
    console.log("result : ", result);
