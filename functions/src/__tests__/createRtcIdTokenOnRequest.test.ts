    ...
    test('500 ERROR : verifyFirebaseAuthIdToken', async () => {
        ...
        const response = {
            ...
            json: (json: Result<string>) => {
                ...
                expect(JSON.parse(json.data).uid).toEqual(JSON.parse(verifyFirebaseAuthIdTokenResult.data).uid);
                expect(JSON.parse(json.data).firebaseAuthIdToken).toEqual(JSON.parse(verifyFirebaseAuthIdTokenResult.data).firebaseAuthIdToken);
            },
        };
    ...
    test('200 OK', async () => {
        ...
        const response = {
            ...
            json: (json: Result<string>) => {
                ...
                expect(JSON.parse(json.data).rtcIdTokenType).toEqual(body.rtcIdTokenType);
                const expectRole = body.role === "publisher" ? 1 : body.role === "audience" ? 2 : undefined;
                expect(JSON.parse(json.data).role).toEqual(expectRole);
                const aboutPrivilegeExpireTime = ValidateRequestData.calcPrivilegeExpireTime(body.privilegeExpireTime)
                expect(JSON.parse(json.data).privilegeExpireTime).toBeWithinRange(aboutPrivilegeExpireTime - 5000, aboutPrivilegeExpireTime + 500);

                expect(JSON.parse(json.data).firebaseAuthIdToken).toEqual(body.firebaseAuthIdToken);
                expect(JSON.parse(json.data).verifyFirebaseAuthUid).toEqual(JSON.parse(verifyFirebaseAuthIdTokenResult.data).uid);
                expect(JSON.parse(json.data).rtcIdToken).not.toEqual(JSON.parse(verifyFirebaseAuthIdTokenResult.data).rtcIdToken);
