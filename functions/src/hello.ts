...
export const helloWorldSecureOnRequest = functions.https.onRequest(
  async (request, response) => {
    functions.logger.info(
      "helloWorldSecure logs start -------------------------------------------------- ",
      { structuredData: true }
    );

    // Firebase Authentication で認証状況確認
    const firebaseAuthIdToken = String(request.query.token);
    const verifyFirebaseAuthIdTokenResult: Result<string> =
      await verifyFirebaseAuthIdTokenOnRequest(firebaseAuthIdToken);

    functions.logger.info(verifyFirebaseAuthIdTokenResult, {
      structuredData: true,
    });
    ...
    functions.logger.info(
      "helloWorldSecure logs end -------------------------------------------------- ",
      { structuredData: true }
    );
  }
);

export const helloWorldSecureOnCall = functions.https.onCall(
  async (data, context) => {
    functions.logger.info(
      "helloWorldSecureOnCall logs start -------------------------------------------------- ",
      { structuredData: true }
    );
    ...
    if (context.auth) {
      result = {
        ...
        context: { ...context, auth: !context.auth },
    ...
    functions.logger.info("helloWorldSecureOnCall auth end", {
      structuredData: true,
    });
    ...
    functions.logger.info(
      "helloWorldSecureOnCall logs end -------------------------------------------------- ",
      { structuredData: true }
    );
  }
);
